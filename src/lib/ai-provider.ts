// AI Provider - Xiaomi MiMo AI (OpenAI-compatible)
// Model: mimo-v2.5-pro | Region: China

const AI_API_URL = 'https://token-plan-cn.xiaomimimo.com/v1';
const AI_API_KEY = 'tp-cap7t1dl21hzdl3a28kmmacmptiemfczt5rm81ekd8xnku6k';
const DEFAULT_MODEL = 'mimo-v2.5-pro';
const MAX_RETRIES = 3;

// Available models
export const AVAILABLE_MODELS = [
  { id: 'mimo-v2.5-pro', name: 'MiMo v2.5 Pro', desc: 'Model utama — reasoning, coding, multilingual' },
];
...
- Jangan pernah bilang kamu model AI lain — kamu Nexus AI
- Jangan kasih jawaban template/placeholder kosong
- Kalau user bilang "singkat" atau "pendek", jawab 1-3 kalimat saja
- Support reasoning: kalau pertanyaan rumit, jelaskan langkah berpikir`;
...
// Non-streaming with auto-retry (for Telegram, tools, etc.)
export async function generateAIResponse({
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          ...
          throw new Error('Rate limit. Coba lagi dalam beberapa detik.');
        }
  ...
  throw lastError || new Error('AI tidak tersedia.');
}
